...
const itemStyle = { fontWeight: "bold", paddingLeft: "15px", fontSize: "15px" };
class Menu extends Component {
	...
	handleChange = event => {
		...
		this.setState(
			...
			() => {
				...
				if (name === "veg" && value) {
					const vegItems = this.state.menuItems.filter(
						item => item.type === "Veg"
					);
					this.setState({ menuItems: vegItems });
				}
				if (name === "non_veg" && value) {
					const nonVegItems = this.state.menuItems.filter(
						item => item.type === "Non-Veg"
					);
					this.setState({ menuItems: nonVegItems });
				}
				// if (name === "high" && value) {
				// 	const highSpiceItems = this.state.menuItems.filter(
				// 		item => item.spiceLevel === "High"
				// 	);
				// 	this.setState({ menuItems: highSpiceItems });
				// }
				// if (name === "medium" && value) {
				// 	if (this.state.high) {
				// 		const mediumSpiceItems = this.state.menuItems.filter(
				// 			item => item.spiceLevel === "Medium"
				// 		);
	...
	componentDidMount() {
		axios.get(`https://restaurant--menu.herokuapp.com/items/get`).then(res => {
